...
export async function getSingleCategory(id: string): Promise<CategoryInterface> {
    try {
        const data = await fetch(`https://ecommerce.routemisr.com/api/v1/categories/${id}`)
        if (!data.ok) throw new Error("Error occured, please try again.")
...
export async function getProductsByCategory(categoryId: string): Promise<productsInterface[]> {
    try {
        const data = await fetch(`https://ecommerce.routemisr.com/api/v1/products?category=${categoryId}&limit=20`)
        if (!data.ok) throw new Error("Error occured, please try again.")
